...
public class PostPaymentCollection : BaseEndpoint<Payment>
{
    ...
    private async Task<IResult> ProcessRequest([FromRoute] string debtCode,
                                               [FromBody] IEnumerable<Payment> payments,
                                               ...
                                               HttpContext context)
    {
        ...
        {
            var result = await ProcessPayment(debt,
                                              payment,
                                              unitOfWork);
    ...
    private async Task<ProcessPaymentResult> ProcessPayment(Debt debt,
                                                            Payment payment,
                                                            IUnitOfWork unitOfWork)
    {
        var charge = await unitOfWork.DebtDetailUserRepository.Find(payment.DebtDetailUser.Id);
